...
// Redirect if timetable_id is not set
...
class TimetableSummaryGenerator {
    ...
    // Get template ID for a timetable
    public function getTemplateId($timetable_id) {
        $stmt = $this->conn->prepare("
            SELECT template_id FROM t1 
            WHERE timetable_id = ? 
            LIMIT 1
        ");
        $stmt->bind_param("s", $timetable_id);
        $stmt->execute();
        $result = $stmt->get_result();
        return $result->num_rows > 0 ? $result->fetch_assoc()['template_id'] : null;
    }
        ...
        <div class="timetable-summary">
            ...
            <form id="summary-form" method="POST" action="">
                ...
                <div class="form-actions" style="margin-top: 20px;">
                    ...
                    <a href="next_33.php?timetable_id=<?php echo urlencode($summary_data['summary']['timetable_id']); ?>&template_id=<?php echo urlencode($summary_data['summary']['template_id']); ?>" class="staff-availability-btn">Staff Availability</a>
                </div>
            </form>
        </div>
...
try {
    ...
    // Always generate summary on page load (since we have the timetable_id from session)
    try {
        $generator = new TimetableSummaryGenerator($conn);
...
<body>
    <div class="container">
        <a href="next_11.php" class="back-btn">← Back to Course Assignment</a>
